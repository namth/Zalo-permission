/**
 * /api/admin/workspaces/[id]/tools/[toolId]/data/[dataId]
...
export async function PUT(
    req: NextRequest,
    { params }: { params: { id: string; toolId: string; dataId: string } }
    ...
    try {
        ...
        logger.error(`PUT workspace/${params.id}/tools/${params.toolId}/data/${params.dataId} error: ${error}`);
...
export async function DELETE(
    ...
    try {
        ...
        logger.error(`DELETE workspace/${params.id}/tools/${params.toolId}/data/${params.dataId} error: ${error}`);
        return NextResponse.json({ success: false, error: String(error) }, { status: 500 });
    }
}
